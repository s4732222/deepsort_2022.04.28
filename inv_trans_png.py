...
img2 = cv2.imread('C:\\Users\\ADMIN\\Desktop\\yolov4-deepsort\\outputs\\taichung_roi_ipm_0424(3)(4).jpg') #轉置後的圖 用來反轉置

#pts = np.array([[1124,543], [1260,543], [1293,578], [1140,575]], dtype=np.float32) 
...
#ipm_pts = np.array([[1124,543], [1260,543], [1260,575], [1124,575]], dtype=np.float32)
#ipm_pts = np.array([[429,584], [896,584], [896,612], [429,610]], dtype=np.float32)#中清路line12
#ipm_pts = np.array([[430,587], [898,588], [898,694], [430,694]], dtype=np.float32)#中清路line12(new)
...
#ipm_pts = np.array([[657,616], [750,616], [750,677], [657,677]], dtype=np.float32)#台灣大道line12(new)
...
inv_ipm_matrix = cv2.getPerspectiveTransform(ipm_pts, pts) #反轉置 #反矩陣 = cv2.getPerspectiveTransform(新點, 原點)
print(inv_ipm_matrix)


#ipm = cv2.warpPerspective(img, ipm_matrix, img.shape[:2][::-1]) 
inv_ipm = cv2.warpPerspective(img2, inv_ipm_matrix, img2.shape[:2][::-1]) #原圖 = cv2.warpPerspective(透視轉換的圖, 反矩陣轉置矩陣, img.shape[:2][::-1])




#print(img.shape[:2][::-1])
print(img2.shape[:2][::-1])

# display (or save) images
cv2.imshow('img', img)
#cv2.imshow('ipm', ipm) #轉置
cv2.imshow('inv_ipm', inv_ipm) #反轉置
#cv2.imwrite("C:\\Users\\ADMIN\\Desktop\\yolov4-deepsort\\outputs\\taichung_ipm_0421(3)(4).jpg",ipm)
cv2.imwrite("C:\\Users\\ADMIN\\Desktop\\yolov4-deepsort\\outputs\\taichung_roi_ipm_inv_0424(3)(4).jpg",inv_ipm)
cv2.waitKey()
